...
void worker_thread_function (FIFORequestChannel* rqc, BoundedBuffer &req_buf, BoundedBuffer &resp_buf, string filename) {
    ...
    while(true) {
        ...
        if (mes == DATA_MSG) {
            ...
            // send the req across the FIFO channel to server
            ...
            // send message across FIFO channel
            int len = sizeof(filemsg) + filename.size() + 1;
            rqc->cwrite(buf, len);
            // collect response
...
int main (int argc, char* argv[]) {
    ...
    vector<FIFORequestChannel*> req_channels = {};
    ...
    if (f.size() == 0) {
        ...
        for (int i = 0; i < w; i++) {
            // create a channel for the worker thread (store in FIFO array)
